...
<div class="col-12 interviewBookingsRow d-none p-0">
   <div class="job-box-info interview-box clearfix">
      <div class="box-head">
         <h4 class="text-white">1: Interview from {{$UserInterview->employer->company}}</h4>
      </div>
              ...
              @if ($quest->video_response == 1)
                  <div class="video_response"> 
                    <p> Upload video to answer this question </p> 
                      <input type="file" class="question_id form-control-file" id="exampleFormControlFile1" name="answer[{{$quest->id}}]" accept="video/mp4,video/x-m4v,video/*">
                  </div>
                @else
                <input type="text" class="w100 mt10 form-control jobseekersAnswer_{{ $loop->index }}" name="answer[{{$quest->id}}]">
              @endif
            </div>
