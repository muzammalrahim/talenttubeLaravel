...
this.bulkremoveButton = function(){
  ...
  $('.bulkRemoveUser .cbx_list').html(cbx_hidden);
  $('.bulkRemoveUser').submit();


  /*var formData = $('.bulkRemoveUser').serializeArray();
  console.log(formData);
  $.ajaxSetup({headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')}});
  $.ajax({
    type: 'POST',
    url: '/admin/ajax/bulk/removeTracker',
    // url: '{!! route('users.removeTracker') !!}',ajax/bulk/removeTracker
    data: {'data': formData},
    success: function(data) {
      // console.log(' data ', data);
...
this.sendOnlineTestFunction = function(){
    ...
    $.ajax({
        type: 'POST',
        url: base_url+'/admin/ajax/online/bulkTest/send',
        data:formData,
        success: function(response){
            if(response.status == 1){
                var message = response.message;
