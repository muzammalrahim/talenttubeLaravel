...
//  ======================================= Edit Employer Questions End  =======================================
    ...
    this.confirmPhotoDelete = function(gallery_id){
        ...
        $.ajax({
            type: 'POST',
            url: base_url+'/admin/ajax/deleteGallery/'+gallery_id+'/'+x,
            success: function(data){
                $('#photo_'+gallery_id).removeAttr("src");
                $('#photo_'+gallery_id).remove();
                $('#photodiv_'+gallery_id).remove();
            }
        });
    }
    ...
    this.setPrivateAccess = function(gallery_id){
        ...
        $.ajax({
            type: 'POST',
            url: base_url+'/admin/ajax/setGalleryPrivateAccess/'+gallery_id,
            success: function(data){
                if(data.status == 1){
    ...
    this.deleteAttachment = function(attachment_id){
        ...
        $.ajax({
            type:'GET',
            url: base_url+'/admin/ajax/removeAttachment/',
            data: {attachment_id: attachment_id, userID: x},
            success: function(data){
                console.log(' data ', data);
                if(data.status == 1){
    ...
    this.deleteVideoConfirm = function(video_id){
        var x = document.getElementById("formUserId").value;
        $.ajaxSetup({headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')}});
        $.ajax({
            type: 'POST',
            url: base_url+'/admin/ajax/deleteVideo',
            data: {video_id: video_id, userID: x},
            success: function(data){
                if(data.status == 1){
                    $('#v_'+video_id).remove();
                }
    ...
    // tab script
     ...
     $('.uploadProgressModalBtn').on('click', function() {
			...
			input.onchange = e => {
                ...
                var that = this;
               var item_id =  Math.floor((Math.random() * 1000) + 1);
               var gallery_item = '<div class="imageDiv" id="photodiv_'+item_id+'" style="display: inline-block;">';
                gallery_item += '<img data-photo-id="'+item_id+'" id="photo_'+item_id+'" class="photo" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAAAXNSR0IArs4c6QAAAARnQU1BAACxjwv8YQUAAAAJcEhZcwAADsQAAA7EAZUrDhsAAAANSURBVBhXYzh8+PB/AAffA0nNPuCLAAAAAElFTkSuQmCC"\>';
                gallery_item += '<div class="img-overlay text-center">';
                gallery_item += '<button onclick="UProfile.confirmPhotoDelete('+item_id+'); return false;" class="btn btn-sm btn-success">Delete</button>'
                gallery_item +='</div></div>';
                console.log("form data", formData);
               $('.imagesDiv').append(gallery_item);
               $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                }
			    ...
			    $.ajax({
			       url: base_url+'/admin/ajax/uploadUserGallery',
			       type : 'POST',
			       data : formData,
			       ...
			       success : function(resp) {
			          ...
			          if(resp.status == 1){
                            $('#photodiv_'+item_id).replaceWith(resp.html);
